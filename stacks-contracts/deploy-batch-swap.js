/**
 * Deploy batch-swap contract to Stacks Mainnet
 *
 * Usage:
 *   node deploy-batch-swap.js
 *
 * Requires:
 *   STACKS_PRIVATE_KEY in .env  (64-char hex, no 0x prefix)
 *
 * After deployment:
 *   Update BATCH_SWAP_CONTRACT in frontend/lib/helpers/batch-swap.ts
...
const CONTRACT_NAME = 'batch-swap';
...
async function deploy() {
  const contractPath = path.join(__dirname, 'contracts', 'batch-swap.clar');
  ...
  console.log('\n🚀  Deploying batch-swap to Stacks Mainnet');
  console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
  ...
  const tx = await makeContractDeploy({
    ...
    fee:              100000,    // 0.1 STX — sufficient for 14KB contract at current fee rates
    nonce,
  });

  console.log('📡  Broadcasting...\n');

  // Serialize manually and POST so we can see the raw error
  const txBytes = tx.serialize(); // Uint8Array in v6
  ...
  const outPath = path.join(__dirname, 'batch-swap-deployment.json');
  fs.writeFileSync(outPath, JSON.stringify(info, null, 2));
  console.log('💾  Saved to:', outPath);

  console.log('\n📝  Next steps:');
  console.log(`    1. Wait for confirmation (~10 min): ${explorerUrl}`);
  console.log(`    2. Update BATCH_SWAP_CONTRACT in frontend/lib/helpers/batch-swap.ts:`);
