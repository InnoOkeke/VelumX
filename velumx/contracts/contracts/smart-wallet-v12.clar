  ...
  (let (
    ...
    (try! 
      (if (is-eq target .paymaster-module-v10)
          ;; Branch A: Bridge - Call burn directly from Smart Wallet
          (let ((args (unwrap! (from-consensus-buff? { amount: uint, fee: uint, recipient: (buff 32) } payload) ERR-DECODE-FAILED)))
              ;; Call burn as the Smart Wallet contract (which holds the USDCx)
              (as-contract (contract-call? 'ST1PQHQKV0RJXZFY1DGX8MNSNYVE3VGZJSRTPGZGM.usdcx-v1 burn (get amount args) u0 (get recipient args)))
          )
          ;; Branch B: Fallback (Must also return (response bool uint))
          (begin
             (print { event: "v12-generic-execution", target: target, payload: payload })
             (ok true)
          )
      )
    )
