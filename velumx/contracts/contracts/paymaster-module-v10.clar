...
(define-public (settle-fee 
  ...
  (let (
    ...
    (try! (contract-call? token-trait transfer base-cost-usdcx contract-caller relayer none))
    (if (> treasury-fee u0)
        (try! (contract-call? token-trait transfer treasury-fee contract-caller (var-get treasury) none))
        false
    )
    (ok true)
  )
